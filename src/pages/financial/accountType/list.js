...
export default function ListAccountType(props) {
    ...
    const filter = (item) => {
        ...
        return (
            ...
            (
                normalizeString(item?.nome_tipo)?.toLowerCase().includes(normalizedFilterData?.toLowerCase())
            )
        );
    };
    ...
    return (
        <>
            <SectionHeader
                title={`Tipos de Conta (${accountTypesList?.filter(filter)?.length || '0'})`}
            ...
            <ContentContainer sx={{ display: { xs: 'none', sm: 'none', md: 'flex', lg: 'flex', xl: 'flex' } }}>
                <Box sx={{ display: 'flex', flex: 1, justifyContent: 'space-between' }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                        ...
                        <Text bold style={{ color: '#d6d6d6' }} light>{accountTypesList?.filter(filter)?.length || '0'}</Text>
                        <Text style={{ color: '#d6d6d6' }} light>de</Text>
                        <Text bold style={{ color: '#d6d6d6' }} light>{accountTypesList?.length || 0}</Text>
                        <Text style={{ color: '#d6d6d6' }} light>Tipos de Conta</Text>
                    </Box>
                </Box>
            ...
            {accountTypesList?.length > 0 ?
                ...
                <Box sx={{ alignItems: 'center', justifyContent: 'center', display: 'flex', padding: '80px 40px 0px 0px' }}>
                    <Text bold>Não conseguimos encontrar Tipos de conta cadastrados</Text>
                </Box>
            }
            {accountTypesList?.length > 0 &&
