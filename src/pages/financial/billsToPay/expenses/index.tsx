...
export default function Expenses() {
    ...
    const fetchReportData: ({ page, limit }: FetcherData) => Promise<void> = async ({ page = 0, limit = 15 }: FetcherData) => {
        ...
        try {
            ...
            if (data.length > 0) {
                console.log(data)
                setExpenses(data.map((item: Expenses) => {
                    const value = typeof item.valor_desp === 'string' ? formatterLiquidValue(parseFloat(item.valor_desp)) : formatterLiquidValue(item.valor_desp)
                    return {
                        ...item,
                        valor_desp: value
                    }
        ...
        const total = data
            .map(item => {
                if (typeof item.valor_desp === 'number') {
                    ...
                    const numericValue = parseFloat(item.valor_desp.replace(/\./g, '').replace(',', '.'));
    ...
    const handleUpdateExpenses = async () => {
        if (expensesSelected || expensesSelectedExclude) {
            try {
                ...
                const expensesSelect = isToUpdate && expensesList?.filter(item => item.id_despesa && isToUpdate.includes(parseInt(item.id_despesa)))
                ...
                if (expensesSelect && expensesSelect.length > 0) {
                    for (let expense of expensesSelect) {
                        const response = await api.patch(`/expense/update/processData`, { expenseData: expense, userRespId: user?.id })
                ...
                {expensesList.length > 0 ?
                    <Box sx={{ opacity: loadingData ? .6 : 1 }}>
                        ...
                        <Box sx={styles.boxValueTotally}>
                            <ButtonIcon text="Processar" icon={'/icons/process.png'} color="#fff" onClick={() => handleUpdateExpenses()} />
                            <Box>
                                <Text title light>Total Pendente: </Text>
                                <Text title bold>{formatReal(calculationTotal(expensesList))}</Text>
                            </Box>
                        </Box>
                    </Box>
                    :
